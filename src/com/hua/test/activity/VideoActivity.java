...
//	private String[] itemTitles = { "热点", "娱乐", "搞笑", "精品" };
	...
	public void initContentView() {
		...
		// item_width = (int) (mScreenWidth / itemCount + 0.5f); //
		// 一个Item宽度为屏幕的1/7
	...
	public OnPageChangeListener pageListener = new OnPageChangeListener() {
			...
			if (state == ViewPager.SCROLL_STATE_DRAGGING) {
				...
				if (mViewPager.getCurrentItem() == currentFragmentIndex) {
					...
					animation = new TranslateAnimation(endPosition,
							currentFragmentIndex * scroll_distance, 0, 0);
		...
		public void onPageSelected(int position) {
			...
			if (animation != null) {
				...
				animation.setDuration(bottom_indicate_line_duration);
				buttom_indicate_line.startAnimation(animation);
				// mColumnHorizontalScrollView.smoothScrollTo((currentFragmentIndex
				// - 1) * temp_scroll_distance , 0);
				mColumnHorizontalScrollView.smoothScrollTo(
						temp_scroll_distance, 0);
			}
	...
	private void initColumnData() {
		...
		for (String title : MyConstants.Video_Item_Titles) {
	...
	private void initTabColumn() {
		...
		for (int i = 0; i < count; i++) {
			...
			final FrameLayout.LayoutParams mRadioGroup_contentParams = new FrameLayout.LayoutParams(
					mScreenWidth, LayoutParams.WRAP_CONTENT);
			...
			if (!isGet_Scroll_Distance) {
				...
				vto2.addOnGlobalLayoutListener(new OnGlobalLayoutListener() {
					...
					public void onGlobalLayout() {
						columnTextView.getViewTreeObserver()
