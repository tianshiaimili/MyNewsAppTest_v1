...
public class NbaAdapter extends BaseAdapter {
    ...
    private static NbaAdapter mNewAdapter;
...
//    	LogUtils2.i("newIndex = "+newIndex+"   oldIndex = "+oldIndex);
        if (list != null  && !lists.contains(list.get(0)) && list.size() > 0 && newIndex != oldIndex) {
...
//            LogUtils2.e("*********lists.size==***== " +lists.size());
    ...
    public static NbaAdapter getYuLeAdapter(Context tempContext){
    	
    	if(mNewAdapter == null){
    		mNewAdapter = new NbaAdapter(tempContext);
    	}
    	return mNewAdapter;
    	
    }
    
    public NbaAdapter (Context tempContext){
    	if(tempContext != null){
    		context = tempContext;
    	}
