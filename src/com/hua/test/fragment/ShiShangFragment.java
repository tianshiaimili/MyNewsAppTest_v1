...
public class ShiShangFragment extends BaseFragment implements
	...
	// @Bean
	protected ShiShangAdapter adapter;
	...
	protected void initContentView(View tempContentView) {
		...
		adapter = ShiShangAdapter.getAdapter(mContext);
		...
		// /load data
		loadData(getCommonUrl(index + "", Url.ShiShangId), cacheName);
	...
	public void getResult(String result) {
		if (result != null) {
			...
			List<NewModle> list = NewListJson.instance(getActivity())
					.readJsonNewModles(result, Url.ShiShangId);
	...
	public void onRefresh() {
		new Handler().postDelayed(new Runnable() {
			...
			public void run() {
				...
				isRefresh = true;
				index = 0;
				loadData(getCommonUrl(0 + "", Url.ShiShangId), cacheName);
				url_maps.clear();
