...
import com.hua.test.adapter.news.NbaAdapter;
...
public class NBAFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener,
	...
	private String cacheName = "NBAFragment";// this.getClass().getSimpleName();

	//当前fragment 在viewpage中的第几个页面下的index
	private int mTabIndex;
    
	public NBAFragment(int tabIndex) {
		super();
...
//		}
		...
		adapter = NbaAdapter.getYuLeAdapter(mContext);
		...
		// /load data
		loadData(getCommonUrl(index + "", Url.NBAId), cacheName);
...
//					.readJsonNewModles(result, Url.NBAId);
...
//    						Url.TopId);
    		...
    		List<NewModle> list =JacksonJsonUtil.readJson2NewModles(result, Url.NBAId);
    		/////
	...
	public void onRefresh() {
		new Handler().postDelayed(new Runnable() {
			...
			public void run() {
				...
				isRefresh = true;
				index = 0;
				loadData(getCommonUrl(0 + "", Url.NBAId), cacheName);
				url_maps.clear();
