...
import com.hua.test.adapter.news.DianYingAdapter;
...
public class DianYingFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener,
	...
	protected DianYingAdapter Adapter;
	...
	private String cacheName = "DianYingFragment";// this.getClass().getSimpleName();

	//当前fragment 在viewpage中的第几个页面下的index
	private int mTabIndex;
    
	public DianYingFragment(int tabIndex) {
		super();
...
//		
		...
		Adapter = DianYingAdapter.getYuLeAdapter(mContext);
		...
		// /load data
		loadData(getCommonUrl(index + "", Url.DianYingId), cacheName);
		...
		mSwipeListView.setOnItemClickListener(new MyDYListViewItemListener());
...
//    						Url.TopId);
    		...
    		List<NewModle> list =JacksonJsonUtil.readJson2NewModles(result, Url.DianYingId);
    		/////
	...
	public void onRefresh() {
		new Handler().postDelayed(new Runnable() {
			...
			public void run() {
				...
				isRefresh = true;
				index = 0;
				loadData(getCommonUrl(0 + "", Url.DianYingId), cacheName);
				url_maps.clear();
...
//          new GetDataTask().execute(url);
	...
	class MyDYListViewItemListener implements OnItemClickListener {
