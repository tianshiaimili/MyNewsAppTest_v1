...
import com.hua.test.adapter.news.YiDongAdapter;
...
public class YiDongFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener,
	...
	protected YiDongAdapter adapter;
	...
	private String cacheName = "YiDongFragment";// this.getClass().getSimpleName();

	//当前fragment 在viewpage中的第几个页面下的index
	private int mTabIndex;
    
	public YiDongFragment(int tabIndex) {
		super();
...
//		}
		...
		adapter = YiDongAdapter.getYuLeAdapter(mContext);
		...
		// /load data
		loadData(getCommonUrl(index + "", Url.YiDongId), cacheName);
...
//					.readJsonNewModles(result, Url.YiDongId);
...
//    						Url.TopId);
    		...
    		List<NewModle> list =JacksonJsonUtil.readJson2NewModles(result, Url.YiDongId);
    		/////
	...
	public void onRefresh() {
		new Handler().postDelayed(new Runnable() {
			...
			public void run() {
				...
				isRefresh = true;
				index = 0;
				loadData(getCommonUrl(0 + "", Url.YiDongId), cacheName);
				url_maps.clear();
