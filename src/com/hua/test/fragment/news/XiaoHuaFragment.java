...
import com.hua.test.adapter.news.ShiShangAdapter;
import com.hua.test.adapter.news.XiaoHuaAdapter;
...
public class XiaoHuaFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener,
	...
	// @Bean
	protected XiaoHuaAdapter adapter;
	...
	private String cacheName = "ShiShangFragment";// this.getClass().getSimpleName();
	...
	protected void initContentView(View tempContentView) {
		...
		mProgressBar = (ProgressBar) tempContentView
		...
		adapter = XiaoHuaAdapter.getAdapter(mContext);
		...
		// /load data
		loadData(getCommonUrl(index + "", Url.XiaoHuaId), cacheName);
...
//					.readJsonNewModles(result, Url.XiaoHuaId);
...
//    						Url.TopId);
    		...
    		List<NewModle> list =JacksonJsonUtil.readJson2NewModles(result, Url.XiaoHuaId);
    		/////
	...
	public void onRefresh() {
		new Handler().postDelayed(new Runnable() {
			...
			public void run() {
				...
				isRefresh = true;
				index = 0;
				loadData(getCommonUrl(0 + "", Url.XiaoHuaId), cacheName);
				url_maps.clear();
