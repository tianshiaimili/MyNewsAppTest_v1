...
import com.hua.test.adapter.news.ShouJiAdapter;
import com.hua.test.adapter.news.YouXiAdapter;
...
//  @Bean
  protected YouXiAdapter adapter;
...
//		}
		...
		adapter = YouXiAdapter.getAdapter(mContext);
		...
		// /load data
		loadData(getCommonUrl(index + "", Url.YouXiId), cacheName);
...
//					.readJsonNewModles(result, Url.YouXiId);
...
//    						Url.TopId);
    		...
    		List<NewModle> list =JacksonJsonUtil.readJson2NewModles(result, Url.YouXiId);
    		/////
	...
	public void onRefresh() {
		new Handler().postDelayed(new Runnable() {
			...
			public void run() {
				...
				isRefresh = true;
				index = 0;
				loadData(getCommonUrl(0 + "", Url.YouXiId), cacheName);
				url_maps.clear();
