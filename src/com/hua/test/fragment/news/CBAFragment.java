...
import com.hua.test.adapter.news.CBAAdapter;
...
//		}
		...
		adapter = CBAAdapter.getAdapter(mContext);
		...
		// /load data
		loadData(getCommonUrl(index + "", Url.CBAId), cacheName);
...
//					.readJsonNewModles(result, Url.CBAId);
...
//    						Url.TopId);
    		...
    		List<NewModle> list =JacksonJsonUtil.readJson2NewModles(result, Url.CBAId);
    		/////
	...
	public void onRefresh() {
		new Handler().postDelayed(new Runnable() {
			...
			public void run() {
				...
				isRefresh = true;
				index = 0;
				loadData(getCommonUrl(0 + "", Url.CBAId), cacheName);
				url_maps.clear();
