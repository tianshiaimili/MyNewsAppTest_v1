...
import com.hua.test.adapter.news.QiCheAdapter;
import com.hua.test.adapter.news.SheHuiAdapter;
...
//  @Bean
  protected SheHuiAdapter adapter;
...
//		}
		...
		adapter = SheHuiAdapter.getAdapter(mContext);
		...
		// /load data
		loadData(getCommonUrl(index + "", Url.SheHuiId), cacheName);
...
//					.readJsonNewModles(result, Url.SheHuiId);
...
//    						Url.TopId);
    		...
    		List<NewModle> list =JacksonJsonUtil.readJson2NewModles(result, Url.SheHuiId);
    		/////
	...
	public void onRefresh() {
		new Handler().postDelayed(new Runnable() {
			...
			public void run() {
				...
				isRefresh = true;
				index = 0;
				loadData(getCommonUrl(0 + "", Url.SheHuiId), cacheName);
				url_maps.clear();
