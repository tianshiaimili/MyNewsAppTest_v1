...
import com.hua.test.adapter.news.LvYouAdapter;
...
//  @Bean
  protected LvYouAdapter adapter;
...
//		}
		...
		adapter = LvYouAdapter.getAdapter(mContext);
		...
		// /load data
		loadData(getCommonUrl(index + "", Url.LvYouId), cacheName);
...
//					.readJsonNewModles(result, Url.LvYouId);
...
//    						Url.TopId);
    		...
    		List<NewModle> list =JacksonJsonUtil.readJson2NewModles(result, Url.LvYouId);
    		/////
	...
	public void onRefresh() {
		new Handler().postDelayed(new Runnable() {
			...
			public void run() {
				...
				isRefresh = true;
				index = 0;
				loadData(getCommonUrl(0 + "", Url.LvYouId), cacheName);
				url_maps.clear();
