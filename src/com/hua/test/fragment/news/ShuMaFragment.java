...
import com.hua.test.adapter.news.ShuMaAdapter;
...
public class ShuMaFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener,
	...
	protected ShuMaAdapter adapter;
	...
	private String cacheName = "ShuMaFragment";// this.getClass().getSimpleName();

	//当前fragment 在viewpage中的第几个页面下的index
	private int mTabIndex;
    
	public ShuMaFragment(int tabIndex) {
		super();
...
//		}
		...
		adapter = ShuMaAdapter.getYuLeAdapter(mContext);
		...
		// /load data
		loadData(getCommonUrl(index + "", Url.ShuMaId), cacheName);
	...
	public void getResult(String result) {
		if (result != null) {
			...
			List<NewModle> list = NewListJson.instance(getActivity())
					.readJsonNewModles(result, Url.ShuMaId);
			mProgressBar.setVisibility(View.GONE);
			if (index == 0) {
	...
	public void onRefresh() {
		new Handler().postDelayed(new Runnable() {
			...
			public void run() {
				...
				isRefresh = true;
				index = 0;
				loadData(getCommonUrl(0 + "", Url.ShuMaId), cacheName);
				url_maps.clear();
