...
import com.hua.test.adapter.news.JiaJuAdapter;
...
//  @Bean
  protected JiaJuAdapter adapter;
...
//		}
		...
		adapter = JiaJuAdapter.getAdapter(mContext);
		...
		// /load data
		loadData(getCommonUrl(index + "", Url.JiaJuId), cacheName);
...
//					.readJsonNewModles(result, Url.JiaJuId);
...
//    						Url.TopId);
    		...
    		List<NewModle> list =JacksonJsonUtil.readJson2NewModles(result, Url.JiaJuId);
    		/////
	...
	public void onRefresh() {
		new Handler().postDelayed(new Runnable() {
			...
			public void run() {
				...
				isRefresh = true;
				index = 0;
				loadData(getCommonUrl(0 + "", Url.JiaJuId), cacheName);
				url_maps.clear();
