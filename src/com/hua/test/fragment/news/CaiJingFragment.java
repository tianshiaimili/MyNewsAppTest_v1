...
import com.hua.test.adapter.news.CaiJingAdapter;
...
public class CaiJingFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener,
	...
	protected CaiJingAdapter adapter;
	...
	private String cacheName = "CaiJingFragment";// this.getClass().getSimpleName();

	//当前fragment 在viewpage中的第几个页面下的index
	private int mTabIndex;
    
	public CaiJingFragment(int tabIndex) {
		super();
...
//		showProgressDialog();
		...
		adapter = CaiJingAdapter.getYuLeAdapter(mContext);
		...
		// /load data
		loadData(getCommonUrl(index + "", Url.CaiJingId), cacheName);
		...
		mSwipeListView.setOnItemClickListener(new MyListViewItemListener());
...
//			dismissProgressDialog();
	...
	private void initSliderLayout(List<NewModle> newModles) {
		...
		LogUtils2.e("");
		adapter.appendList(newModles, index);
	}
...
//					.readJsonNewModles(result, Url.CaiJingId);
...
//    						Url.TopId);
    		...
    		List<NewModle> list =JacksonJsonUtil.readJson2NewModles(result, Url.CaiJingId);
    		/////
			...
			if (index == 0) {
				...
				adapter.appendList(list, index);
			}

//			dismissProgressDialog();

			if (adapter.isNeedUplistsModlesData(index)) {
	...
	public void onRefresh() {
		new Handler().postDelayed(new Runnable() {
			...
			public void run() {
				...
				isRefresh = true;
				index = 0;
				loadData(getCommonUrl(0 + "", Url.CaiJingId), cacheName);
				url_maps.clear();
...
//          new GetDataTask().execute(url);
	...
	class MyListViewItemListener implements OnItemClickListener {
