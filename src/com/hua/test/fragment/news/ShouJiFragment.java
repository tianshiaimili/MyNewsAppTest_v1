...
//  @Bean
  protected ShouJiAdapter adapter;
...
//		}
		...
		adapter = ShouJiAdapter.getAdapter(mContext);
		...
		// /load data
		loadData(getCommonUrl(index + "", Url.ShouJiId), cacheName);
...
//					.readJsonNewModles(result, Url.ShouJiId);
...
//    						Url.TopId);
    		...
    		List<NewModle> list =JacksonJsonUtil.readJson2NewModles(result, Url.ShouJiId);
    		/////
	...
	public void onRefresh() {
		new Handler().postDelayed(new Runnable() {
			...
			public void run() {
				...
				isRefresh = true;
				index = 0;
				loadData(getCommonUrl(0 + "", Url.ShouJiId), cacheName);
				url_maps.clear();
