...
//   @Bean
   protected QiCheAdapter adapter;
...
//		}
		...
		adapter = QiCheAdapter.getAdapter(mContext);
		...
		// /load data
		loadData(getCommonUrl(index + "", Url.QiChiId), cacheName);
...
//					.readJsonNewModles(result, Url.QiChiId);
...
//    						Url.TopId);
    		...
    		List<NewModle> list =JacksonJsonUtil.readJson2NewModles(result, Url.QiChiId);
    		/////
	...
	public void onRefresh() {
		new Handler().postDelayed(new Runnable() {
			...
			public void run() {
				...
				isRefresh = true;
				index = 0;
				loadData(getCommonUrl(0 + "", Url.QiChiId), cacheName);
				url_maps.clear();
