...
import com.hua.test.adapter.news.KeJiAdapter;
...
public class KeJiFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener,
	...
	protected KeJiAdapter kejiAdapter;
	...
	private String cacheName = "KeJiFragment";// this.getClass().getSimpleName();

	//当前fragment 在viewpage中的第几个页面下的index
	private int mTabIndex;
    
	public KeJiFragment(int tabIndex) {
		super();
...
//		}
		...
		kejiAdapter = KeJiAdapter.getYuLeAdapter(mContext);
		...
		// /load data
		loadData(getCommonUrl(index + "", Url.KeJiId), cacheName);
...
//			dismissProgressDialog();
	...
	private void initSliderLayout(List<NewModle> newModles) {
		...
		LogUtils2.e("");
		kejiAdapter.appendList(newModles, index);
	}
...
//					.readJsonNewModles(result, Url.KeJiId);
...
//    						Url.TopId);
    		...
    		List<NewModle> list =JacksonJsonUtil.readJson2NewModles(result, Url.KeJiId);
    		/////
			...
			if (index == 0) {
				...
				kejiAdapter.appendList(list, index);
			}

//			dismissProgressDialog();

			if (kejiAdapter.isNeedUplistsModlesData(index)) {
	...
	public void onRefresh() {
		new Handler().postDelayed(new Runnable() {
			...
			public void run() {
				...
				isRefresh = true;
				index = 0;
				loadData(getCommonUrl(0 + "", Url.KeJiId), cacheName);
				url_maps.clear();
